...
# meaning that it contains alphanumeric characters (which includes letters, numbers, and underscores), as well as periods, dashes, and a plus sign,
# followed by a period and a character-only top-level domain such as ".com", ".info", ".edu", etc.
# Fill in the regular expression to do that, using escape characters, wildcards, repetition qualifiers, beginning and end-of-line characters,
...
# no leading zero, followed by a colon, then minutes between 00 and 59, then an optional space, and then AM or PM, in upper/lower case.
# Fill in the regular expression to do that. How many of the concepts that you just learned can you use here?


def check_time(text):
    pattern2 = r"(?i)^(1[0-2]|0?[1-9]):[0-5][0-9]\s?(AM|PM)$"
...
# hould return True since (IM) satisfies the match conditions." Fill in the regular expression in this function:
...
def check_zip_code(text):
    result = re.search(
        r".*\s[0-9][0-9][0-9][0-9][0-9]-?[0-9]?[0-9]?[0-9]?[0-9]?", text)
    return result != None
